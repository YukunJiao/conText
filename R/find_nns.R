...
find_nns <- function(target_embedding, pre_trained, N = 5, candidates = NULL, norm = "l2", stem = FALSE, language = 'porter'){
  if(length(candidates) > 0) pre_trained <- pre_trained[candidates, , drop = FALSE]
  cos_sim <- text2vec::sim2(x = pre_trained, y = matrix(target_embedding, nrow = 1), method = "cosine", norm = norm)[,1]
  ...
  return(utils::head(nn_df$token, N))
}
